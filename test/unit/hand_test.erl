...
-define(MK_RANK(Cards), hand:rank(hand:new(hand:make_cards(Cards)))).
-define(MK_PLAYER_HAND(Cards), hand:player_hand(?MK_RANK(Cards))).
-define(SHORT(Hand), {Hand#hand.rank, Hand#hand.high1, Hand#hand.high2, Hand#hand.score}).
...
rank_high_card1_test() ->
    H = ?MK_RANK("4D JH 5D 8C QD TD 7H"),
    ?assertMatch(#hand{rank = ?HC_HIGH_CARD, high1 = 2#00111011000000}, H).

rank_high_card2_test() ->
    H = ?MK_RANK("8C AD 5H 3S KD 9D 4D"),
    ?assertMatch(#hand{rank = ?HC_HIGH_CARD, high1 = 2#11000110010000}, H).

rank_high_card3_test() ->
    H = ?MK_RANK("4C JH 5C 8D QC 2C 3D"),
    ?assertMatch(#hand{rank = ?HC_HIGH_CARD, high1 = 2#00110010011000}, H).
    
rank_pair1_test() ->
    H = ?MK_RANK("KD 3S 5H 3D 6C QH 9S"),
    ?assertEqual(?HC_PAIR, H#hand.rank),
    ?assertEqual(2#00000000000100, H#hand.high1),
    ?assertEqual(2#01100100000000, H#hand.score).

rank_pair2_test() ->
    H = ?MK_RANK("AC 2D 5D AS 4H 9D KD"),
    ?assertEqual(?HC_PAIR, H#hand.rank),
    ?assertEqual(2#10000000000000, H#hand.high1),
    ?assertEqual(2#01000100010000, H#hand.score).

rank_pair3_test() ->
    H = ?MK_RANK("9S JH 5D TS 3C KC 3H"),
    ?assertEqual(?HC_PAIR, H#hand.rank),
    ?assertEqual(2#00000000000100, H#hand.high1),
    ?assertEqual(2#01011000000000, H#hand.score).

rank_two_pair1_test() ->
    H = ?MK_RANK("QC KD JD QD JC 5C KC"),
...
rank_two_pair2_test() ->
    H = ?MK_RANK("7H 3H 6C TD 7C JH 6H"),
    ?assertEqual(?HC_TWO_PAIR, H#hand.rank),
...
rank_two_pair3_test() ->
    H = ?MK_RANK("4D 3S 5H JD JC QH 5S"),
    ?assertEqual(?HC_TWO_PAIR, H#hand.rank),
...
rank_two_pair4_test() ->
    H = ?MK_RANK("AC 2D 5D AS 5H 9D 4D"),
    ?assertEqual(?HC_TWO_PAIR, H#hand.rank),
    ?assertEqual(2#10000000000000, H#hand.high1),
    ?assertEqual(2#00000000010000, H#hand.high2),
...
rank_two_pair5_test() ->
    H = ?MK_RANK("9S JH 5D JS 5C KC 3D"),
    ?assertEqual(?HC_TWO_PAIR, H#hand.rank),
    ?assertEqual(2#00010000000000, H#hand.high1),
    ?assertEqual(2#00000000010000, H#hand.high2),
...
rank_three_kind1_test() ->
    H = ?MK_RANK("KH 9S 5H QD QC QH 3S"),
...
rank_full_house2_test() ->
    H = ?MK_RANK("AC AD KD AS KH 9D 4D"),
    ?assertEqual(?HC_FULL_HOUSE, H#hand.rank),
    ?assertEqual(2#10000000000000, H#hand.high1),
...
rank_full_house3_test() ->
    H = ?MK_RANK("3S JH JD JS KH KC 5D"),
    ?assertEqual(?HC_FULL_HOUSE, H#hand.rank),
    ?assertEqual(2#00010000000000, H#hand.high1),
    ?assertEqual(2#01000000000000, H#hand.high2),
    ?assertEqual(0, H#hand.score).
...
rank_full_house4_test() ->
    H = ?MK_RANK("TD QH TH TC 6C QD QC"),
    ?assertEqual(?HC_FULL_HOUSE, H#hand.rank),
    ?assertEqual(2#00100000000000, H#hand.high1),
...
rank_four_kind1_test() ->
    H = ?MK_RANK("4D AS 5H QD QC QH QS"),
    ?assertEqual(?HC_FOUR_KIND, H#hand.rank),
    ?assertEqual(2#00100000000000, H#hand.high1),
    ?assertEqual(2#10000000000000, H#hand.score).

rank_four_kind2_test() ->
    H = ?MK_RANK("AC KC KD KS KH 9D 4D"),
    ?assertEqual(?HC_FOUR_KIND, H#hand.rank),
    ?assertEqual(2#01000000000000, H#hand.high1),
    ?assertEqual(2#10000000000000, H#hand.score).

rank_four_kind3_test() ->
    H = ?MK_RANK("KS TS QD QS QH QC 5D"),
    ?assertEqual(?HC_FOUR_KIND, H#hand.rank),
    ?assertEqual(2#00100000000000, H#hand.high1),
    ?assertEqual(2#01000000000000, H#hand.score).

rank_straight_flush1_test() ->
    H = ?MK_RANK("KC QC JC TC 9C 4D AS"),
    ?assertEqual(?HC_STRAIGHT_FLUSH, H#hand.rank),
    ?assertEqual(2#01111100000000, H#hand.high1),
    ?assertEqual(0, H#hand.score).

rank_straight_flush2_test() ->
    H = ?MK_RANK("AC KC QC JC TC 9D 4D"),
    ?assertEqual(?HC_STRAIGHT_FLUSH, H#hand.rank),
    ?assertEqual(2#11111000000000, H#hand.high1),
    ?assertEqual(0, H#hand.score).

rank_straight_flush3_test() ->
    H = ?MK_RANK("KS QS JS TS 9S AD 7S"),
    ?assertEqual(?HC_STRAIGHT_FLUSH, H#hand.rank),
    ?assertEqual(2#01111100000000, H#hand.high1),
    ?assertEqual(0, H#hand.score).

rank_straight_flush4_test() ->
    H = ?MK_RANK("3S 4S 5S 6S 7S 8S 9S"),
    ?assertEqual(?HC_STRAIGHT_FLUSH, H#hand.rank),
    ?assertEqual(2#00000111110000, H#hand.high1),
    ?assertEqual(0, H#hand.score).

%%% TEST WIN

high_card_win_test() ->
    H1 = ?MK_RANK("4D JH 5D 8C QD TD 7H"),
    H2 = ?MK_RANK("8C AD 5H 3S KD 9D 4D"),
    H3 = ?MK_RANK("4C JH 5C 8D QC 2C 3D"),
...
high_card_win2_test() ->
    H1 = ?MK_RANK("5C 7D 2H 3S KD 9D AD"),
    H2 = ?MK_RANK("5D 6H 2H 3S KD 9D AD"),
    ?assertEqual(?HC_HIGH_CARD, H1#hand.rank),
    ?assertEqual(?HC_HIGH_CARD, H2#hand.rank),
    ?assertEqual(true, ?SHORT(H1) > ?SHORT(H2)).

high_card_win3_test() ->
  H1 = ?MK_RANK("3H 7S TD QD JD"),
  H2 = ?MK_RANK("5H 6S TD QD JD"),
  ?assertEqual(?HC_HIGH_CARD, H1#hand.rank),
  ?assertEqual(?HC_HIGH_CARD, H2#hand.rank),
  ?assertEqual(true, ?SHORT(H1) > ?SHORT(H2)).

pair_win_test() ->
    H1 = ?MK_RANK("KD 3S 5H 3D 6C QH 9S"),
    H2 = ?MK_RANK("AC 2D 5D AS 4H 9D KD"),
    H3 = ?MK_RANK("9S JH 5D TS 3C KC 3H"),
...
two_pair_win_test() ->
    H1 = ?MK_RANK("4D 3S 5H JD JC QH 5S"),
    H2 = ?MK_RANK("AC 2D 5D AS 5H 9D 4D"),
    H3 = ?MK_RANK("9S JH 5D JS 5C KC 3D"),
...
three_kind_win_test() ->    
    H1 = ?MK_RANK("KH 9S 5H QD QC QH 3S"),
    H2 = ?MK_RANK("AC KC KD KS 7H 9D 4D"),
    H3 = ?MK_RANK("KS TS QD QS QH 4C 5D"),
...
straight_win_test() ->
    H1 = ?MK_RANK("KC QS JH TC 9C 4D 3S"),
    H2 = ?MK_RANK("AC KS QH JC TC 9D 4D"),
    H3 = ?MK_RANK("KS QD JS TC 9S 2D 7S"),
...
flush_win_test() ->
    H1 = ?MK_RANK("4D JD 5D JC QD 2D 7H"),
    H2 = ?MK_RANK("8C AD 5D AS KD 9D 4D"),
    H3 = ?MK_RANK("4C JC 5C 8D QC 3C 7S"),
    H4 = ?MK_RANK("4C JC 7C 8D QC 5C 7S"),
...
four_kind_win_test() ->
    H1 = ?MK_RANK("4D AS 5H QD QC QH QS"),
    H2 = ?MK_RANK("AC KC KD KS KH 9D 4D"),
    H3 = ?MK_RANK("KS TS QD QS QH QC 5D"),
...
straight_flush_win_test() ->
    H1 = ?MK_RANK("KC QC JC TC 9C 4D AS"),
    H2 = ?MK_RANK("AC KC QC JC TC 9D 4D"),
    H3 = ?MK_RANK("KS QS JS TS 9S AD 7S"),
...
full_house_win_test() ->
    H1 = ?MK_RANK("4D JS 5H JD JC QH QS"),
    H2 = ?MK_RANK("AC AD KD AS KH 9D 4D"),
    H3 = ?MK_RANK("3S JH JD JS KH KC 5D"),
...
two_pair_win1_test() ->
    H1 = ?MK_RANK("5C TC 7H KH 5S TS KS"),
    H2 = ?MK_RANK("5C TC 7H KH 5S KC TH"),
    ?assertEqual(?HC_TWO_PAIR, H1#hand.rank),
    ?assertEqual(?HC_TWO_PAIR, H2#hand.rank),
    ?assertEqual(true, ?SHORT(H1) == ?SHORT(H2)).

high_card_win1_test() ->
    H1 = ?MK_RANK("KH TC 9H 7D 6H 5D 2S"),
    H2 = ?MK_RANK("KH TC 9H 7H 6H 3D 2S"),
    ?assertEqual(?HC_HIGH_CARD, H1#hand.rank),
    ?assertEqual(?HC_HIGH_CARD, H2#hand.rank),
    ?assertEqual(true, ?SHORT(H1) == ?SHORT(H2)).

full_house_win1_test() ->
  H1 = ?MK_RANK("2H 2C 5H 5S 5C 7C 4D"),
  H2 = ?MK_RANK("2H 2C 5H 5S 5D 4D 2D"),
  ?assertEqual(?HC_FULL_HOUSE, H1#hand.rank),
  ?assertEqual(?HC_FULL_HOUSE, H2#hand.rank),
  ?assertEqual(true, ?SHORT(H1) == ?SHORT(H2)).
